...
|---|---|---|---|
| **0.5.1** | **Dados/R0** — schema `research_*`, ingestão parametrizável MT5→DB (candles + ticks com agressor), barras canônicas M1→derivadas determinísticas, snapshot/hash, quality checks, Data Health, isolamento técnico | — | **letscode=true ✅** |
| **0.5.2** | OFI/cubo rápido (R2) — OFI assinado, ρ defasado event-time, event study líquido condicionado por liquidez, decaimento | fast | Founder após 0.5.1 |
