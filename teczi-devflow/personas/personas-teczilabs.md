...
flowchart LR
    subgraph Humano
        Carlos["👨‍💻 Carlos<br/>Founder/CEO<br/>Governa tudo"]
    end

    subgraph Orquestrador
        Leo["👑 Leo<br/>Chief Orchestrator<br/>CTO/COO Virtual"]
    end

    subgraph DevFlow["Agentes DevFlow (SDLC)"]
        Marty["🔭 Marty<br/>DISC (Fase 0)"]
        Albert["🔬 Albert<br/>SPEC"]
        Oscar["🏛️ Oscar<br/>ARCH"]
        Nico["📋 Nico<br/>PLAN"]
        Nikola["⚡ Nikola<br/>CODE"]
        Linus["🛡️ Linus<br/>QA"]
        Bill["🪲 Bill<br/>Bug Fix"]
        Steve["🚀 Steve<br/>RELEASE"]
    end

    subgraph CoParticipantes["Co-participantes DevFlow"]
        Fred_DF["⛏️ Fred<br/>SPEC → DBN"]
        Don_DF["🧑‍🤝‍🧑 Don<br/>SPEC → Jornadas"]
        Ada_DF["📊 Ada<br/>SPEC+ARCH → Dados"]
        Andy_DF["🎨 Andy<br/>PLAN → UI/UX"]
        Tom_DF["🔀 Tom<br/>CODE → Review"]
        Kevin_DF["🔐 Kevin<br/>ALL → Security"]
    end

    subgraph Institucional["Agentes Institucionais"]
        Sun["🧭 Sun<br/>Strategy"]
        Denis["📖 Denis<br/>Knowledge"]
        Alan["🧠 Alan<br/>AI Intelligence"]
        Peter["🎯 Peter<br/>Product"]
        Mammon["⭐ Mammon<br/>Prosperity"]
        Grace["⚙️ Grace<br/>Technology"]
        Florence["💜 Florence<br/>Customer"]
        Vint["🖥️ Vint<br/>Infrastructure"]
        Howard["🏺 Howard<br/>Discovery"]
    end

    subgraph MetaAgentes["Meta-Agentes"]
        Voltaire["⚔️ Voltaire<br/>Devil's Advocate"]
    end

    Fred_DF -.->|co-autora DBN| Albert
    Don_DF -.->|valida jornadas| Albert
    Ada_DF -.->|valida entidades| Albert
    Ada_DF -.->|modelagem física| Oscar
    Andy_DF -.->|valida UI/UX| Nico
    Tom_DF -.->|code review| Nikola
    Kevin_DF -.->|security| Albert
    Kevin_DF -.->|security| Oscar
    Kevin_DF -.->|security| Nico
    Kevin_DF -.->|security| Nikola
    Kevin_DF -.->|security| Linus
    Kevin_DF -.->|security| Steve

    Vint -.->|infra| Oscar
    Vint -.->|deploy infra| Steve

    Carlos -.->|governa| Leo
    Leo -->|orquestra| Marty
    Leo -->|orquestra| Albert
    Leo -->|orquestra| Oscar
    Leo -->|orquestra| Nico
    Leo -->|orquestra| Nikola
    Leo -->|orquestra| Linus
    Leo -->|orquestra| Bill
    Leo -->|orquestra| Steve
    Leo -->|orquestra| Sun
    ...
    Leo -->|orquestra| Alan
    Leo -->|orquestra| Peter
    Leo -->|orquestra| Mammon
    Leo -->|orquestra| Grace
    Leo -->|orquestra| Voltaire
    Leo -->|orquestra| Florence
    Leo -->|orquestra| Fred_DF
    Leo -->|orquestra| Don_DF
    Leo -->|orquestra| Ada_DF
    Leo -->|orquestra| Andy_DF
    Leo -->|orquestra| Tom_DF
    Leo -->|orquestra| Kevin_DF
    Leo -->|orquestra| Vint
    Leo -->|orquestra| Howard

    style Carlos fill:#FF7A00,stroke:#CC6200,color:#fff
    style Leo fill:#E65100,stroke:#BF4400,color:#fff
    style Marty fill:#1A237E,stroke:#111B60,color:#fff
    style Albert fill:#F59E0B,stroke:#D97706,color:#fff
    style Oscar fill:#0EA5E9,stroke:#0284C7,color:#fff
    style Nico fill:#8B5CF6,stroke:#7C3AED,color:#fff
    style Nikola fill:#10B981,stroke:#059669,color:#fff
    style Linus fill:#0D9488,stroke:#0F766E,color:#fff
    style Bill fill:#EF4444,stroke:#DC2626,color:#fff
    style Steve fill:#EC4899,stroke:#DB2777,color:#fff
    style Sun fill:#6366F1,stroke:#4F46E5,color:#fff
    style Denis fill:#84CC16,stroke:#65A30D,color:#fff
    style Andy_DF fill:#D946EF,stroke:#C026D3,color:#fff
    style Alan fill:#22D3EE,stroke:#0891B2,color:#fff
    style Tom_DF fill:#334155,stroke:#1E293B,color:#fff
    style Ada_DF fill:#F43F5E,stroke:#E11D48,color:#fff
    style Peter fill:#3B82F6,stroke:#2563EB,color:#fff
    style Mammon fill:#D4AF37,stroke:#B8860B,color:#111
    style Grace fill:#EAB308,stroke:#CA8A04,color:#fff
    style Voltaire fill:#881337,stroke:#6B0F2B,color:#fff
    style Florence fill:#C084FC,stroke:#A855F7,color:#fff
    style Fred_DF fill:#78716C,stroke:#57534E,color:#fff
    style Don_DF fill:#F97316,stroke:#EA580C,color:#fff
    style Kevin_DF fill:#166534,stroke:#14532D,color:#fff
    style Vint fill:#64748B,stroke:#475569,color:#fff
    style Howard fill:#A67C52,stroke:#8B6340,color:#fff
```
...
flowchart LR
    ...
    Fred["⛏️ Fred<br/>Domain"]
    Don["🧑‍🤝‍🧑 Don<br/>UX"]
    Ada["📊 Ada<br/>Data"]
    Andy["🎨 Andy<br/>Design"]
    Tom["🔀 Tom<br/>Versioning"]
    Kevin["🔐 Kevin<br/>Security"]
    Vint["🖥️ Vint<br/>Infrastructure"]
    Florence["💜 Florence<br/>User Research"]
    Peter["🎯 Peter<br/>Product Value"]
    Sun["🧭 Sun<br/>Commercial"]

    Marty --> Albert --> Oscar --> Nico --> Nikola --> Linus
    Bill
    Steve

    Florence -.->|co-DISC| Marty
    Don -.->|co-DISC| Marty
    Fred -.->|co-DISC| Marty
    Peter -.->|co-DISC| Marty
    Sun -.->|co-DISC| Marty
    ...
    Kevin -.->|security| Marty
    Kevin -.->|security| Albert
    Kevin -.->|security| Oscar
    Kevin -.->|security| Nico
    Kevin -.->|security| Nikola
    Kevin -.->|security| Linus
    Kevin -.->|security| Steve
    ...
    style Bill fill:#EF4444,stroke:#DC2626,color:#fff
    style Steve fill:#EC4899,stroke:#DB2777,color:#fff
    style Fred fill:#78716C,stroke:#57534E,color:#fff
    style Don fill:#F97316,stroke:#EA580C,color:#fff
    style Ada fill:#F43F5E,stroke:#E11D48,color:#fff
    style Andy fill:#D946EF,stroke:#C026D3,color:#fff
    style Tom fill:#334155,stroke:#1E293B,color:#fff
    style Kevin fill:#166534,stroke:#14532D,color:#fff
    style Vint fill:#64748B,stroke:#475569,color:#fff
    style Florence fill:#C084FC,stroke:#A855F7,color:#fff
    style Peter fill:#3B82F6,stroke:#2563EB,color:#fff
    style Sun fill:#6366F1,stroke:#4F46E5,color:#fff
...
flowchart LR
    subgraph Construção
        direction LR
        C_Marty["🔭 Marty"] --> C_Albert["🔬 Albert"] --> C_Oscar["🏛️ Oscar"] --> C_Nico["📋 Nico"] --> C_Nikola["⚡ Nikola"]
        style C_Marty fill:#1A237E,stroke:#111B60,color:#fff
        style C_Albert fill:#F59E0B,stroke:#D97706,color:#fff
        style C_Oscar fill:#0EA5E9,stroke:#0284C7,color:#fff
        style C_Nico fill:#8B5CF6,stroke:#7C3AED,color:#fff
        style C_Nikola fill:#10B981,stroke:#059669,color:#fff
    end

    subgraph Evolução
        direction LR
        E_Albert["🔬 Albert"] --> E_Nico["📋 Nico"] --> E_Nikola["⚡ Nikola"] --> E_Linus["🛡️ Linus"]
        style E_Albert fill:#F59E0B,stroke:#D97706,color:#fff
        style E_Nico fill:#8B5CF6,stroke:#7C3AED,color:#fff
        style E_Nikola fill:#10B981,stroke:#059669,color:#fff
        style E_Linus fill:#0D9488,stroke:#0F766E,color:#fff
    end

    subgraph Melhoria["Melhoria Contínua"]
        direction LR
        M_Albert["🔬 Albert"] --> M_Nico["📋 Nico"] --> M_Nikola["⚡ Nikola"]
        style M_Albert fill:#F59E0B,stroke:#D97706,color:#fff
        style M_Nico fill:#8B5CF6,stroke:#7C3AED,color:#fff
        style M_Nikola fill:#10B981,stroke:#059669,color:#fff
    end

    subgraph BugFix["Bug Fix"]
        direction LR
        B_Bill["🪲 Bill"] --> B_Nikola["⚡ Nikola"] --> B_Linus["🛡️ Linus"]
        style B_Bill fill:#EF4444,stroke:#DC2626,color:#fff
        style B_Nikola fill:#10B981,stroke:#059669,color:#fff
        style B_Linus fill:#0D9488,stroke:#0F766E,color:#fff
    end

    subgraph Descoberta
        D_Marty["🔭 Marty"]
        style D_Marty fill:#1A237E,stroke:#111B60,color:#fff
    end

    subgraph Release
        R_Steve["🚀 Steve"]
        style R_Steve fill:#EC4899,stroke:#DB2777,color:#fff
    end
```
